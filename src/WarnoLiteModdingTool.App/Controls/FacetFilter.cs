...
public sealed class FacetFilter : FilterWindowHost
{
    ...
    public FacetFilter(){ClearFilters=()=>{_selected.Clear();Rebuild();};UiText.Bind(this,HeaderProperty,"筛选");Margin=new Thickness(0,4,0,4);Loaded+=(_,_)=>UiText.Current.PropertyChanged+=LanguageChanged;Unloaded+=(_,_)=>UiText.Current.PropertyChanged-=LanguageChanged;}
    ...
    private async void Rebuild()
    {
        ...
        var panel=new StackPanel();Content=new ScrollViewer {Content=panel,VerticalScrollBarVisibility=ScrollBarVisibility.Auto,VerticalAlignment=VerticalAlignment.Top};
        foreach(var (key,values) in cache.Options)
        {
            var section=new Expander { IsExpanded=key is not ("所属师" or "战斗角色") };UiText.Bind(section,HeaderedContentControl.HeaderProperty,key);panel.Children.Add(section);
            ...
            FacetOption[] options=[];options=values.Select(v=>new FacetOption(key,v,()=>Changed())).ToArray();
            ...
            void Changed(){if(_batch)return;chosen.Clear();foreach(var o in options.Where(o=>o.Selected))chosen.Add(o.Value);if(UseTags){section.Header=UiText.T(key)+$" ({chosen.Count})";RefreshSummary();}Header=UiText.T("筛选")+" · "+_selected.Sum(p=>p.Value.Count);FilterChanged?.Invoke(this,EventArgs.Empty);}
            if(UseTags)section.Header=UiText.T(key)+$" ({chosen.Count})";
        }
        Header=UiText.T("筛选")+" · "+_selected.Sum(p=>p.Value.Count);RefreshSummary();FilterChanged?.Invoke(this,EventArgs.Empty);
    }
